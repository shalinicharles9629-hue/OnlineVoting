    ...
    return (
        <div className="min-h-screen bg-gov-bg p-6 md:p-12">
            <div className="container mx-auto max-w-6xl">
                ...
                >
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800">
                            {t('dashboard.welcome')}, <span className="text-gov-blue">{user?.name}</span>
                        </h1>
                        <p className="text-gray-500 mt-1 capitalize">{t('dashboard.role')}: <span className="font-semibold text-gov-orange">{user?.role}</span></p>
                    </div>
                    <div className="mt-4 md:mt-0">
                        <div className="bg-blue-50 text-gov-blue px-6 py-3 rounded-xl font-bold border border-blue-100 flex items-center gap-2">
                            <span>🆔</span> {t('dashboard.id_verified')}
                        </div>
                    </div>
                </motion.div>
                ...
                {user?.role === 'admin' && (
                    ...
                    >
                        <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
                            <span className="bg-gov-blue text-white w-8 h-8 rounded-full flex items-center justify-center text-sm">🛠</span> {t('dashboard.admin_panel')}
                        </h2>
                        <div className="grid md:grid-cols-2 gap-6">
                            <div className="bg-white p-8 rounded-xl shadow-md border-l-4 border-green-500 hover:shadow-lg transition">
                                <h3 className="text-xl font-bold mb-2">{t('dashboard.create_election')}</h3>
                                <p className="text-gray-500 mb-6">{t('dashboard.create_election_desc')}</p>
                                <Link to="/admin/create-election" className="block text-center bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold w-full transition">
                                    + {t('dashboard.create_election')}
                                </Link>
                            </div>
                            <div className="bg-white p-8 rounded-xl shadow-md border-l-4 border-yellow-500 hover:shadow-lg transition">
                                <h3 className="text-xl font-bold mb-2">{t('dashboard.manage_candidates')}</h3>
                                <p className="text-gray-500 mb-6">{t('dashboard.manage_candidates_desc')}</p>
                    ...
                    >
                        <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                            <div className="bg-gov-orange p-4 text-white font-bold text-lg flex justify-between items-center">
                                <span>{t('dashboard.candidate_portal')}</span>
                                {application?.symbol && (
                                    <span className="bg-white/20 px-3 py-1 rounded-lg text-sm">{t('dashboard.assigned_symbol')}: {application.symbol}</span>
                                )}
                            </div>
                            <div className="p-8">
                                <div className="flex flex-col md:flex-row items-center gap-6">
                                    <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center text-3xl overflow-hidden border-2 border-gray-100">
                                        {application?.photoUrl ? (
                                            <img src={`http://localhost:5000${application.photoUrl}`} alt={t('dashboard.me_alt')} className="w-full h-full object-cover" />
                                        ) : "👤"}
                                    </div>
                                    <div className="flex-1 text-center md:text-left">
                                        <h3 className="text-xl font-bold text-gray-800">{t('dashboard.app_status')}</h3>
                                        <p className="text-gray-500">
                                            {application
                                                ? t('dashboard.submitted_on', { date: new Date(application.createdAt).toLocaleDateString() })
                                                : t('dashboard.no_app')}
                                        </p>
                                    </div>
                                    <div className="text-center">
                                        {application?.isApproved ? (
                                            <div className="flex flex-col items-end">
                                                <span className="bg-green-100 text-green-800 px-4 py-1 rounded-full text-sm font-bold border border-green-200">{t('dashboard.approved')}</span>
                                                <p className="text-[10px] text-green-600 mt-1 font-bold uppercase tracking-wider">{t('dashboard.ready_election')}</p>
                                            </div>
                                        ) : application?.rejectionReason ? (
                                            <div className="flex flex-col items-end">
                                                <span className="bg-red-100 text-red-800 px-4 py-1 rounded-full text-sm font-bold border border-red-200">{t('dashboard.rejected')}</span>
                                                <p className="text-[10px] text-red-500 mt-1 max-w-[150px] text-right truncate" title={application.rejectionReason}>{t('register.reason', { defaultValue: 'Reason' })}: {application.rejectionReason}</p>
                                            </div>
                                        ) : application ? (
                                            <div className="flex flex-col items-end">
                                                <span className="bg-yellow-100 text-yellow-800 px-4 py-1 rounded-full text-sm font-bold border border-yellow-200 animate-pulse">{t('dashboard.pending')}</span>
                                                <p className="text-[10px] text-yellow-600 mt-1 uppercase font-bold tracking-wider">{t('dashboard.under_review')}</p>
                                            </div>
                                        ) : (
                                            <span className="bg-gray-100 text-gray-500 px-4 py-1 rounded-full text-sm font-bold">{t('dashboard.not_applied')}</span>
                                        )}
                                    </div>
                                </div>
                                {application?.rejectionReason && (
                                    <div className="mt-6 p-4 bg-red-50 border border-red-100 rounded-xl">
                                        <p className="text-red-800 text-sm font-semibold mb-1">{t('dashboard.admin_feedback')}</p>
                                ...
                                <div className="mt-8 pt-6 border-t border-gray-100">
                                    <Link to="/apply" className="text-gov-blue hover:text-blue-800 font-semibold flex items-center gap-2 justify-center border border-dashed border-blue-300 p-4 rounded-lg bg-blue-50 hover:bg-blue-100 transition">
                                        📝 {application ? t('dashboard.view_form') : t('dashboard.complete_form')}
                                    </Link>
                                </div>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                            <div className="text-center md:text-left">
                                <h2 className="text-2xl md:text-3xl font-bold mb-2">{t('dashboard.want_represent')}</h2>
                                <p className="text-blue-100 md:max-w-xl">
                                    {t('dashboard.represent_desc')}
                                </p>
                            </div>
                            <Link
                                to="/apply"
                ...
                <div>
                    <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
                        <span className="bg-gov-blue text-white w-8 h-8 rounded-full flex items-center justify-center text-sm">🗳</span> {t('dashboard.active_elections')}
                    </h2>
                    ...
                    {elections.length > 0 ? (
                            ...
                            {elections.map((election, idx) => (
                                ...
                                >
                                            ...
                                            {election.status === 'ongoing' ? (
                                                ...
                                                >
                                                    {election.status === 'ended' && !election.resultsPublished
                                                        ? t('dashboard.results_pending')
                                                        : election.status === 'upcoming'
                                                            ? t('dashboard.not_started')
                                                            : t('dashboard.view_official')}
                                                </Link>
                                            )}
                                        </div>
                                    </div>
                                    <div className="px-6 py-3 bg-gray-50 border-t border-gray-100 text-xs text-gray-400 flex justify-between items-center rounded-b-xl">
                                        <span>{t('dashboard.id')}: {election._id.slice(-6)}</span>
                                        <span>{t('dashboard.official')}</span>
                                    </div>
                                </motion.div>
                            ))}
                        </div>
                        ...
                        <div className="bg-white p-12 rounded-xl shadow-sm text-center border dashed border-gray-300">
                            ...
                            <h3 className="text-xl font-bold text-gray-400">{t('dashboard.no_elections')}</h3>
                            <p className="text-gray-500">{t('dashboard.no_elections_desc')}</p>
                        </div>
                    )}
                </div>
            </div>
